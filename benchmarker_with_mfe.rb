...
unless ActiveRecord::Base.connection.execute("show databases").map(&:this).flatten.include?("fftbor_performance_with_mfe")
  ActiveRecord::Base.connection.create_database("fftbor_performance_with_mfe")
end

ActiveRecord::Base.establish_connection(config.merge(database: "fftbor_performance_with_mfe"))
